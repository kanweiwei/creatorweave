...
export const BUILTIN_MACROS: Macro[] = [
  ...
  // 4. Review: Code Review Checklist
  {
    ...
    calls: [
      ...
      {
        name: 'grep',
        arguments: { pattern: 'TODO|FIXME|HACK|XXX', file_pattern: '{path}' },
        description: 'Check for TODO/FIXME comments',
      },
      {
        name: 'grep',
        arguments: { pattern: 'console\\.(log|warn|error)', file_pattern: '{path}' },
        description: 'Check for console logs',
      },
      {
        name: 'grep',
        arguments: { pattern: 'any|unknown', file_pattern: '{path}' },
  ...
  // 5. Analysis: Performance Check
  {
    ...
    calls: [
      ...
      {
        name: 'grep',
        arguments: { pattern: 'useEffect.*\\[\\]', file_pattern: '{path}' },
        description: 'Check for useEffect with empty deps',
      },
      {
        name: 'grep',
        arguments: { pattern: 'useState|useMemo|useCallback', file_pattern: '{path}' },
        description: 'Check hook usage patterns',
      },
      {
        name: 'grep',
        arguments: { pattern: 'map.*key=', file_pattern: '{path}' },
